...
from tkinter import Tk, Label, Button, Entry, filedialog, Toplevel
...
class ImageUI:
    def __init__(self, master):
        ...
        self.load_button = Button(master=self.master, text="Load Image", command=self.load_image, width=11, height=2)
        self.load_button.pack()

        self.generate_button = Button(master=self.master, text="Generate Graph", command=self.generate_graph, width=11, height=2)
...
if __name__ == "__main__":
    root = Tk()
    root.iconify()  # 창을 숨기는 방법입니다.
    app = ImageUI(root)
    # root.deiconify()  # 창을 다시 보이게 하는 방법입니다.
    root.mainloop()
